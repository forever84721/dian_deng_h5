...
router.beforeEach((to, from, next) => {
  ...
  if (!ls && to.path !== '/' && to.path.indexOf('login') === -1  && to.path !== '/forget' && to.path.indexOf('register') === -1 && to.path.indexOf('forget') === -1 && to.path !== '/person-detail' && to.path !== '/chooseAreaCode' && to.path !== '/serviceAndPrivacy' && to.path !== '/bindAccount' && to.path !== '/pay' && to.path !== '/orderDetail' && to.path !== '/blessMessage' ) {
    ...
    if((to.path.indexOf('login') !== -1) && localStorage.getItem('ecPayItem')) {
      ...
      console.log('成2')
    } else if((to.path.indexOf('login') !== -1) && localStorage.getItem('ecPayDonateItem')){
      next({
        name:'donate'
      })
      console.log('成3')
    }else if((to.path.indexOf('login') !== -1) && localStorage.getItem('ecPayRenewOrderItem')){
      next({
        name:'orderDetail',
        query: {
      ...
      console.log(JSON.parse(localStorage.getItem('ecPayRenewOrderItem')).orderId);
      console.log('成3')
    }else {
      next()
      console.log('cheng4')
